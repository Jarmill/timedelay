% Optimal Control of a trajectory of a logistic delay differential equation
% x'(t) = -r x(t)(1 - x(t - tau)/K) where the state history is constant
%
% Author: Jared Miller
%         May 27, 2021.
...
sol = dde23(@(t,y,z) T0*r*y*(1-z/K), [tau],@(t) xh0,[0,T], options);
...
%     plot([-tau, T], [0, 0], ':k')
    ...
    xlabel('t')
    ylabel('x(t)')
    title(['$\dot{x}(t) = T0 r x(t) (1 - x(t-', num2str(tau),')/K + u(t)$'], 'interpreter', 'latex', 'fontsize', 16)
end
