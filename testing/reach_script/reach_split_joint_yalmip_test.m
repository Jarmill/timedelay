...
%present [0, T-tau]
Omega0 = struct('ineq', [t*((T-tau) - t); Xdup], 'eq', []);
%present [T-tau, T]
Omega1 = struct('ineq', [(t-(T-tau))*(T-t); Xdup], 'eq', []);


% Xall = struct('ineq', [t*(T-t); Xdup], 'eq', []);
...
Lie_term = Lie - replace(phi1, x, x1);
phi1_shift = replace(phi1, t, t+tau);
phi1_shift_lie = replace(phi1_shift, x, x0);
%Omega 0 constraint [0, T-tau]
[pOm0, consOm0, GramOm0] = psatz(-Lie_term - phi1_shift_lie, Omega0, order, [t; x0; x1]);

%Omega 1 constraint [T-tau, T]
[pOm1, consOm1, GramOm1] = psatz(-Lie_term, Omega1, order, [t; x0; x1]);
...
cons = [cons0:'Time 0'; consT:'Time T'; ...
    consOm1:'Lie Time [T-tau, T]'; consOm0:'Lie Time[0, T-tau]'; consOmneg:'History Time [-tau, 0]'; ...
    consw:'w nonneg'];
